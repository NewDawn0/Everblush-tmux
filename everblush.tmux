...
main() {
	...
	# Statusline
	local wt_enabled
	wt_enabled="$(get_tmux_option "@neverblush_window_tabs_enabled" "off")"
	readonly wt_enabled

	local right_separator
	right_separator="$(get_tmux_option "@everblush_right_separator" "")"
	readonly right_separator

	local left_separator
	left_separator="$(get_tmux_option "@everblush_left_separator" "")"
	readonly left_separator

	local user
	user="$(get_tmux_option "@everblush_user" "off")"
	readonly user

	local host
	host="$(get_tmux_option "@everblush_host" "off")"
	readonly host

	local date_time
	date_time="$(get_tmux_option "@everblush_date_time" "off")"
	readonly date_time
	...
	local show_window
	readonly show_window="#[fg=$thm_red,bg=$thm_bg,nobold,nounderscore,noitalics]$right_separator#[fg=$thm_bg,bg=$thm_pink,nobold,nounderscore,noitalics] #[fg=$thm_fg,bg=$thm_gray] #W #{?client_prefix,#[fg=$thm_red]"
	...
	readonly show_directory_in_window_status_current="#[fg=$thm_bg,bg=$thm_purple] #I #[fg=$thm_fg,bg=$thm_bg] #{b:pane_current_path} "
